...
+ R aceita dados __numéricos__ ("numeric" e "integer" para números inteiros), de __texto__ (_character_), valores lógicos (`TRUE/FALSE`), entre outros
...
+ interseção (AND): `&`
+ pertenecimento: `%in%`
+ a negação funciona igual `!`: "não pertence a" `!a %in% b`
...
ou selecionar o nome da função e clicar F1

Vai abrir na aba __Help__
...
ap <- get_corona_br(dir = caminho, filename = "01-amapa", uf = "AP")
```

+ No help estão os argumentos na ordem correta, e os valores padrão (por _default_) estão indicados. Se a gente não especificasse a função usaria esses valores.  

---
## Data frames 

+ Duas dimensões: `dim()`, `ncol()`, `nrow()`

+ Selecionar colunas e filtrar linhas. Também com cláusulas lógicas afinal.
